...
def soft_mix(src,dst,ratio,margin=2,type="minfit"):
    left=src.shape[1]
    right=0
    up=src.shape[0]
    down=0
    ...
    if type=="minfit":
        radius = min(right-left,down-up)/2
    elif type=="maxfit":
        radius = max(right - left, down - up) / 2
    else:
        print("wrong type!")
        return -1
    b=1/(1-ratio)
    ...
    rate=radius*ratio
    for i in range(up+margin,down-margin):
        for j in range(left+margin,right-margin):
...
def stitch(images, ratio=0.75, reprojThresh=4.0,showMatches=False):
    ...
    cv2.waitKey()
    pic2=soft_mix(result,imageB,0.6,type="maxfit")
    cv2.imshow('p', pic2)
    cv2.waitKey()
...
if __name__ == '__main__':
    a=cv2.imread("1.bmp")#bg
    b=cv2.imread("7.bmp")#inserted pic
    images=[a,b]
